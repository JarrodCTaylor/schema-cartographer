...
(defn query-unindexed-attr [db attr ref-search-limit db-type]
  (if (= :on-prem db-type)
    (map first (peer/query {:query '[:find (pull ?x a)
                            ...
                            :args [db attr [{attr ['*]}]]}))
    (map first (client/q {:query '[:find (pull ?x a)
                                   :in $ ?attr a
                                   :where [?x ?attr]]
                          :offset 1
                          :limit (or ref-search-limit 1000000)
...
(defn query-attr [db attr cardinality ref-search-limit db-type]
  (let [card-many? (= :db.cardinality/many (:db/ident cardinality))
        ...
        return-value (try (with-retry (if (= :on-prem db-type)
                                        (fn []
                                          (peer/index-pull db pull-map))
                                        (fn []
                                          (client/index-pull db pull-map))))
                          (catch Exception e e))]
    (cond
      (= :db.error/attribute-not-indexed (-> return-value ex-data :db/error)) (query-unindexed-attr db attr ref-search-limit db-type)
...
(defn format-attribute [db {:db/keys [ident valueType cardinality unique noHistory isComponent tupleAttrs] :as attr} ref-search-limit db-type]
  (let [attr-query-results (when (= :db.type/ref (:db/ident valueType)) (query-attr db ident cardinality ref-search-limit db-type))
...
(defn assoc-ns-attrs [db res entity attr ref-search-limit db-type]
  (update-in res [:entities entity :ns-attrs] conj (format-attribute db attr ref-search-limit db-type)))

(defn add-namespace [db res entity attr ref-search-limit db-type]
  (assoc-in res [:entities entity] {:namespace entity
                                    :doc "Automatically generated by Explorer"
                                    :referenced-by ()
                                    :ns-attrs [(format-attribute db attr ref-search-limit db-type)]}))

(defn process-entity [db res {:db/keys [ident] :as attr} ref-search-limit db-type]
  (let [entity (keyword "cartographer.entity" (or (namespace ident) "unnamespaced"))]
    (if (-> res :entities entity)
      (assoc-ns-attrs db res entity attr ref-search-limit db-type)
      (add-namespace db res entity attr ref-search-limit db-type))))
...
(defn format-raw-schema-query
  ...
  [db schema ref-search-limit db-type]
  (reduce
    (fn [res {:db/keys [valueType] :as attr}]
      (cond
        (:db.entity/preds attr) (process-entity-predicate res attr)
        valueType (process-entity db res attr ref-search-limit db-type)
...
(defn explore [db schema ref-search-limit db-type]
  (let [formatted-schema (format-raw-schema-query db schema ref-search-limit db-type)
